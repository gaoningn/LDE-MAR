...
class ConvLayer(nn.Sequential):
	def __init__(
		...
		layers.append(EqualConv2d(in_channel, out_channel, kernel_size, padding=padding, stride=stride,
								  bias=bias and not activate))

		if norm is not None:
			if norm.lower() == "gn":
				g = _pick_gn_groups(out_channel, max_groups=gn_max_groups)
				layers.append(nn.GroupNorm(num_groups=g, num_channels=out_channel, eps=1e-6, affine=True))
			elif norm.lower() == "in":
				layers.append(nn.InstanceNorm2d(out_channel, eps=1e-6, affine=True))
			else:
				raise ValueError(f"Unkown norm: {norm}")
...
class ToRGB(nn.Module):
	def __init__(self, in_channel, upsample=True, blur_kernel=[1, 1, 3, 1]):
		super().__init__()
		...
		self.conv = ConvLayer(in_channel, 1, 1)
...
class Noising(nn.Module):
	def __init__(self, style_dim, motion_dim):
		...
		self.weight = nn.Parameter(torch.randn(motion_dim, style_dim))
...
class Direction(nn.Module):
	def __init__(self, style_dim, feature_dim, motion_dim):
		super(Direction, self).__init__()

		self.D_f = nn.Parameter(torch.randn(feature_dim, style_dim) * 0.02)
		self.D_m = nn.Parameter(torch.randn(motion_dim, style_dim) * 0.02)
	...
	def forward(self, alpha: torch.Tensor, beta: torch.Tensor) -> torch.Tensor:
		v_f = alpha @ self.D_f
		v_m = beta @ self.D_m
		z_r2t = self.W(v_f * v_m)
		return z_r2t

	def _ortho_loss_rows(self, D: torch.Tensor) -> torch.Tensor:
		Dn = D / (D.norm(dim=1, keepdim=True) + 1e-8)
		G = Dn @ Dn.t()
		I = torch.eye(G.size(0), device=G.device, dtype=G.dtype)
		return ((G - I) ** 2).mean()
	def orth_loss(self) -> torch.Tensor:
		return self._ortho_loss_rows(self.D_f) + self._ortho_loss_rows(self.D_m)
	def norm_loss(self, target: float = 1.0) -> torch.Tensor:
		lf = ((self.D_f.norm(dim=1) - target) ** 2).mean()
		lm = ((self.D_m.norm(dim=1) - target) ** 2).mean()
		return lf + lm
