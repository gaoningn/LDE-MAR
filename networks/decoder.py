...
from .ops import (ConstantInput, ConvLayer, StyledConv, ToFlow, ToRGB, Direction, Noising)
...
class MotionDirection(nn.Module):
    def __init__(self, style_dim, feature_dim, motion_dim, pure_data=False, dictionary=True):
        super(MotionDirection, self).__init__()
        self.pure_data = pure_data
        self.dictionary = dictionary
        self.direction = Direction(style_dim, feature_dim, motion_dim)
        self.noising = Noising(style_dim, motion_dim)
        self.feature_P = nn.Parameter(torch.rand(1))
    def forward(self, z_s2r, alpha):
        feature_p = torch.sigmoid(self.feature_P)
        if self.dictionary:
            if self.pure_data:
                z_r2t = self.direction(alpha[0], alpha[1])
                # z_s2t = z_s2r + z_r2t
                z_s2t = (feature_p * z_r2t + (1 - feature_p) * z_s2r) * 2
            else:
                z_r2t = self.direction(alpha[0], alpha[1])
                n_r2t = self.noising(alpha[2])
                z_s2t = (feature_p * z_r2t + (1 - feature_p) * z_s2r) * 2 + n_r2t
        else:
            az_r2t = alpha[0]
            bz_r2t = alpha[1]
            z_r2t = az_r2t * bz_r2t
            if self.pure_data:
                ...
                n_r2t = alpha[2]
                z_s2t = z_s2r + z_r2t + n_r2t
        return z_s2t

class Decoder(nn.Module):
    def __init__(self, image_size, style_dim, scale=1):
        super().__init__()

        channels = [512*scale, 256 * scale, 128 * scale, 64 * scale]

        # self.direction = Direction(style_dim, motion_dim)
        # self.direction = MotionDirection(style_dim, motion_dim, pure_data=pure_data)

        self.input = ConstantInput(channels[0], size=int(image_size / 128))  # 4
